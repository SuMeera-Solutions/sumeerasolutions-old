...
const RecordabilityTool = () => {
  ...
  // Recordability assessment state - persists across page switches
  ...
  const handleIncidentSaved = (incidentRecord) => {
    ...
    // Reset incident form but keep recordability state
  };
  ...
  const handleOptionSelect = (option) => {
    ...
    if (option.result) {
      // This option leads to a final result
      setAnswers({
        ...newAnswers,
  ...
  const handleExceptionEvaluation = () => {
    const checkedExceptions = Object.keys(exceptionChecks).filter(k => exceptionChecks[k]);
    
    if (checkedExceptions.length > 0) {
      // Found exceptions - not work-related
      setAnswers({
        ...answers,
        result: 'not_work_related',
        reason: 'Not work-related due to applicable exceptions',
        exceptions: checkedExceptions
      });
    } else {
      // No exceptions - continue as work-related
      setCurrentStep(currentStep + 1);
    }
  };
  ...
  const handleMedicalTreatmentEvaluation = () => {
    ...
    const recordableTreatments = selectedTreatments.filter(treatment => 
      decisionTree[currentStep].options.find(opt => opt.value === treatment)?.recordable
    );

    if (recordableTreatments.length > 0) {
      setAnswers({
        ...answers,
        result: 'recordable',
        reason: 'Medical treatment beyond first aid received',
  ...
  const handleSpecialCasesEvaluation = () => {
    ...
    if (selectedCases.includes('none_special')) {
      // No special cases
      setCurrentStep(currentStep + 1);
    } else {
